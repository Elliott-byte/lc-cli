...
点题面里的 `url` 那一行就能在浏览器里打开该题；按 `o` 效果一样，两个 tab 都能用。（Vim 的题面
面板里点不开——Vim 的终端会把构成链接的那段转义序列吃掉。在那边用 `\o`。）

底栏只放常用的解题循环。`c`（设置）、`d`/`t`（过滤）、`o`（在网页打开）、`D`（跳到每日一题）、
`ctrl+r`（本地刷新）、`R`（重新下载索引）都照常可用——按 `?` 看完整列表。
...
等级始终由你决定：蒙对的、或者偷看过答案的题，不该自己悄悄换来一个月的间隔。
