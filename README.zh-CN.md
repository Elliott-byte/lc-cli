...
| `D` | 跳到今天的每日一题 |
| `space` | 暂停/继续做题计时（暂停时遮住题面） |
| `?` | 列出全部快捷键（含底栏没显示的） |
| `q` | 退出 |

中间那条竖线可以拖动，把宽度分给任意一边；拖到某一边只剩 24 列时会停住。

打开一道题的同时开始计时——TUI 的状态栏和 **Vim 的状态栏都会显示**，毕竟真正做题的时候
你在 Vim 里。在 TUI 里按 `space`、在 Vim 里按 `\z` 都能暂停——暂停时会弹出一个遮罩把题面（和代码）盖住，
停着表还能读题就等于白嫖时间；再按 `space` 继续。提交失败计时照走；提交**通过**时计时停止并报告用时——TUI 的 `s`、
命令行的 `lc submit`、Vim 里的 `\s` 都算。`lc config timer off` 可以整个关掉。
...
两个窗口的状态栏都会列出可用快捷键，窗口窄时会自动省略次要的（`let g:lc_statusline = 0`
