...
shell。值得再看一遍的题可以放进间隔重复刷题本，它会根据你的提交自动记分，并跟着你在几台机器之间走。
...
`ctrl+r` 随时手动刷新——本地即时，不同于会重新下载整个索引的 `R`。
...
**复习自动记分。** 把到期的题重做一遍然后提交：通过就升一级，失败就掉回 1 级、间隔重新开始。
在哪提交都算——TUI 里、shell 里的 `lc submit`、Vim 里的 `\s`。没到期就又做了一遍只算练手，
排期不动。
...
`let g:lc_auto_statement = 0` 改成手动。在代码窗口直接 `:q` 也不会把你困在题面面板里：面板
作为最后一个窗口时会带着 Vim 一起退出，你会回到出发的地方。
...
在 TUI 里按 `c` 打开设置页：工作区、默认语言、编辑器、刷题本仓库和记忆曲线——边打字边预览这条
曲线的含义。`ctrl+s` 保存，`esc` 取消。
