...
浏览题目、阅读题面、写题——用 lc 自带的编辑器，或者用你自己的——然后直接提交到 LeetCode 的真实
判题机，全程不离开 shell。做题时有计时器跑着，每次尝试都能留一张笔记卡片，值得再看一遍的题放进
间隔重复刷题本，跟着你在几台机器之间走。
...
· [做题](#做题) · [笔记](#笔记) · [刷题本](#刷题本) · [跨机器同步](#跨机器同步)
· [Vim](#vim) · [设置](#设置) · [全部命令](#全部命令)
· [文件都放在哪](#文件都放在哪) · [WSL](#wsl)
...
节奏是：移到一道题，`enter` 打开、写、跑样例、提交——循环往复。对已经开始的题按 `enter`
会重新打开你已有的文件，语言也沿用当初选的；当天第一次打开时还会把文件恢复成 starter
code，让这一天从题目本身开始，而不是从上次的答案开始。在**哪里**写由你决定——见
[做题](#做题)。
...
在 `lc config editor builtin`（或者压根没配编辑器）时，`enter` 打开的是 **lc 自己的
编辑器**：左边题面、右边代码、判题结果弹通知、状态栏跑着计时。不挂起、不交接给别的程序。

![内置编辑器](docs/edit.svg)

左侧题面里的 `url` 行在这里也能直接点击打开网页；主界面的 `o` 快捷键不会穿透进来，
所以代码区里按 `o` 仍然只是输入。每次打开编辑器时，题面都会从顶部开始显示。

| 按键 | 作用 |
| --- | --- |
| `ctrl+w h` / `ctrl+w l` | 开启 Vim 键时，移到题面 / 回到代码 |
| `R` / `ctrl+r` | 跑样例 |
| `S` / `ctrl+s` | 提交 |
| `N` / `ctrl+n` | 开关代码下方的笔记分栏 |
| `B` / `ctrl+b` | 弹遮罩暂停计时——表停着时按它就是继续 |
| `T` / `ctrl+g` | 计时归零 |
| `X` | 确认后恢复起始代码；按 `u` 可找回原缓冲区 |
| `ZZ` | 保存返回——和 Run 同排显示在底栏；关掉 Vim 键时则显示 `esc Back` |

Vim Normal/Visual 模式下，底栏使用好按的单键 `R S N B T X`；进入 Insert 后自动换回 Ctrl
组合，避免输入代码时误触；原来的 Ctrl 组合在所有模式下仍然有效。包括 `ZZ Back`
在内的每项底栏命令也都能直接点击；状态栏只保留当前 Vim 模式。
`X Reset code` 会先要求确认，再把当前语言的 LeetCode 起始代码作为一次可撤销编辑恢复；
在保存编辑器之前，它不会直接覆盖磁盘上的解答。
打开或重置编辑器时，光标会停在最后一行真实代码的末尾，不会跳到文件末换行产生的额外空行。
...
`ZZ` 离开，而 `esc` **永远不会退出屏幕**，就是为了让你放心连按。`lc config vimkeys off`
换成普通编辑器（`esc` 返回）。

**更想用自己的编辑器？** `lc config editor vim`（或 `code -w`，或任何命令）就回到"挂起进
编辑器、退出后接着走"的流程，[Vim 插件](#vim)正是为那一侧做的。两条路都是一等公民，切换只要一行。

**做题计时。** 打开题目只是把表**备好**在 00:00，敲下第一个字符才开始走（读题不计时）。提交
失败计时照走，通过则停表并报告用时；离开编辑器会自动暂停——编辑器里的时间才是做题时间。
`lc timer` 在 shell 里看，`lc config timer off` 整个关掉。在 Vim 里表显示在状态栏上：
`space` 开始、`\z` 弹遮罩暂停、`\Z` 归零。

## 笔记

每次尝试都可以留下一张卡片。提交之后按 `ctrl+n`（或跑 `lc note`、在 Vim 里按 `\n`），lc 会在
这道题自己的 `notes.md` 里盖好标题——日期 · 判定 · 语言——然后把光标放到标题下面，而你刚提交的
代码还在眼前。

![笔记卡片](docs/notes.svg)

在题目列表里按 `n` 就能看这道题的卡片，最新的在最上面；再按一次回到题面。它就是题目目录里的一个
markdown 文件，工作区的 git 会连代码一起管着它；`lc review sync` 也会把卡片连同刷题本一起在
机器之间搬运（取并集：两边的卡片都保留，同步永不删卡）。
...
每行都会在题号、等级和到期日旁显示带颜色的难度，左栏缩到最窄时也不会藏起来。

打开题目就是从空白回忆开始：当天第一次打开——无论从 **Review** 还是 **Problems**、无论到没
到期、也无论以前写没写过——lc 都会把上次的答案恢复成原语言的 starter code；当天再次打开则
保留刚写的新答案，今天已经在别处提交过的题也不会被清空。命令行的 `lc edit` 仍会原样保留
已有答案。
...
回到列表时已经有标记了。标记只认第一次提交：第一次没过的话，之后重试通过也不会把红色改成绿色、
掩盖这次遗忘。打完分标记就消失，没打分的话隔夜也会自动褪去——跨天时 TUI 自己翻页，
...
改动仍可随时用 `g` 或下面的命令同步。
同步之间会互相排队，一次只跑一个——连着评好几道题，或者 TUI 正在同步时你在 Vim 里按 `\s`，
不会再有两个同步抢同一个克隆。
...
`:qa`、`:x`、`ZZ`、`\q` 同理。题面中的 `ZZ` 会直接调用 lc 的全部保存退出，不会让只读 terminal
尝试写入自己。有未保存的改动时照样会像原生 Vim 一样拦住你。
...
整个解题文件都会被提交。新建文件直接从 LeetCode 起始代码开始，不再额外加题目标题或 URL 注释；
已有文件不会改动。
