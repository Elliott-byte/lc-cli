...
`ctrl+n` 开关笔记分栏，`ctrl+b` 弹遮罩暂停计时（表停着时按它就是继续），`ctrl+g` 计时归零，
`esc` 保存返回（同时暂停计时，和退出 Vim
...
**笔记。** 每次提交都可以留一张卡片：提交后跑 `lc note`（或 Vim 里按 `\n`），它会在这道题
自己的 `notes.md` 里盖一个标题（日期 · 判定 · 语言），然后进编辑器让你往下写。Vim 里笔记在
代码**下方的分屏**打开——你刚提交的代码就在眼前，边看边写。列表里按 `n` 就能看这道题的卡片，
最新的在最上面。它就是题目目录里的一个 markdown 文件，工作区的 git 会连代码一起管着它；`lc review sync`
也会把卡片连同刷题本一起在两台机器间搬运（取并集：两边的卡片都保留；在一边删掉的卡片会被另一边
补回来——笔记是记录，同步不做减法）。
